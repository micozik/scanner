...
#   —— 被"抓数据的慢模块"挤掉，是排序错误。
# ★★V27.0：永不跳过的模块（2026-08-21 每天都被预算挤掉）★★
# 8/21实测：这5个模块连续多天因"全局预算用尽"被跳过：
#   【我的持仓相关消息】← 持仓个股级利空/利好，8/12佰维事故就是漏这个
#   【埋伏池/热力图/选股器/事件雷达 四大回测】← 规则该不该停用全靠它
#   【买入后复核】← 判断"我当初就买错了"
# ★这5个全是【判断依据】，而且都是纯计算（读本地json+快照），几秒就完。
#   被"抓网络的慢模块"挤掉是排序错误，不是它们不重要。
...
def wr_tag(name):
    ...
    if not v or v[0] is None:
        return "  \u26aa\u3010%s \u00b7 \u6837\u672c\u4e0d\u8db3\u3011\u4ec5\u4f9b\u53c2\u8003" % name
    rate, ok = v
    if ok:
        return "  \U0001f7e2\u3010%s \u00b7 \u5386\u53f2\u80dc\u7387%.1f%%\u3011\u2605\u53ef\u4f5c\u4e3a\u9009\u80a1\u4f9d\u636e\u2605" % (name, rate)
    return ("  \U0001f534\U0001f534\u3010%s \u00b7 %.1f%% \u00b7 \u5df2\u505c\u7528\u3011"
            "\u4ec5\u4f5c\u8bb0\u5f55\uff0c\u2605AI\u4e0d\u8bb8\u62ff\u5b83\u63a8\u8350\u4efb\u4f55\u6807\u7684\u2605" % (name, rate))
...
def scan_skeleton_top():
    w("=" * 60)
    w("🎯【固定输出模板】用户说『抓盘中，美股，龙虎榜，干活』= 按十一节输出")
    w("  1️⃣今天情况 2️⃣盘中 3️⃣大盘 4️⃣板块(跳升+资金+涨幅) 5️⃣个股")
    w("  6️⃣方向 7️⃣新闻(八类+⑤-B美股) 8️⃣资金 9️⃣推荐 🔟操作 1️⃣1️⃣理由")
    w("  ⚠️缺一节=失职。第7️⃣节最常漏，它是①-B的唯一信息源。")
    w("  ⚠️第9️⃣节答不出就写『答不出』，写空比编一个有价值。")
    w("")
    w("🔒【读报告顺序】1跳升榜 2资金 3涨幅连涨 4推演✅验证 5事件雷达")
    w("   6公告雷达(逐条扫持仓名字) 7冷低早+选股器+交叉 8自问最强方向")
    w("   9最后看持仓 🔟推荐前走完检查表六项")
    w("  ★根因：AI带着问题找答案——不问就不看，所以必然漏。")
    w("=" * 60)
# ========== 零、状态门 ==========
    ...
    try:
        ...
        if os.path.exists(_cf):
            ...
            if int(_cv.get("neg_streak", 0)) >= 2:
                w("  🔴🔴【本模块已自动停用】连续{}次回测平均为负"
                  "（最近一次{:+.2f}%）".format(_cv.get("neg_streak"),
                                              _cv.get("last_avg", 0)))
                w("     ★下面的候选【仅作记录】，AI不许拿它当推荐依据★")
                w("     ★规则记分卡：任何规则胜率<45%立即停用")
    except Exception:
        pass
...
def _cold_archive_and_backtest(hits, spot, c_code, c_name, c_price):
    ...
    if len(days) >= 5:
        ...
        if len(recs) < 5:
            ...
            w("       不要照单买，必须配合板块启动信号")
            # ★★V27.0：连续2次平均为负 → 写入停用标记★★
            # 8/21实测：冷低早连续两次回测为负(-1.18%、-2.85%)，
            #   规则记分卡明写"胜率<45%立即停用"，
            #   但它照样每天输出7只候选 → 规则形同虚设。
            try:
                _cf = "reports/cold_low_verdict.json"
                ...
                _cv["neg_streak"] = _neg
                _cv["last"] = now_beijing().strftime("%Y-%m-%d")
                ...
                if _neg >= 2:
                    w("")
                    w("    🔴🔴【冷低早已自动停用】连续{}次回测平均为负".format(_neg))
                    w("       ★本模块的候选名单从今天起【仅作记录】，")
                    w("         AI不许拿它当推荐依据（规则记分卡：胜率<45%立即停用）")
                    w("       ★哪天回测转正，会自动恢复")
            except Exception:
                pass
    else:
...
def scan_take_profit():
    """止盈体系：赚到的钱要落袋（V5.7核心）"""
    w("\n" + "=" * 60)
    w("💎💎【风险监测体系】嗅到危险才卖 · 不是赚够了就卖 💎💎")
    w("=" * 60)
    w("  ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★")
    w("  ★★【卖出的唯一理由：嗅到危险，不是赚够了】★★")
    w("  ★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★")
    w("  用户2026-08-17原话（这是整套系统最重要的一条）：")
    w("    『不是说一到10%就叫我减半，20%就让我全卖掉。』")
    w("    『万一你抓到一只实力非常不错的股票，赚50%甚至翻倍")
    w("      都是很轻松的，那我岂不是白买？』")
    w("    ★★『你有把握我卖掉以后，马上给我抓回一只")
    w("        这么厉害的股票吗？』★★")
    w("    『我让你做的是【分析市场】，是你【嗅到了危险的气息】")
    w("      才让我去卖掉。如果形势一片大好，为什么不继续持有？』")
    w("")
    w("  ⚠️ AI必须记住的算术：")
    w("     卖出 = 放弃一只【驱动还在、正在跑】的票")
    w("          + 需要重新做一次【选股】")
    w("     而选股是AI最差的能力（台账：0胜4负）")
    w("     ★所以『卖了再买更好的』是负期望，除非危险确实出现")
    w("")
    w("  ⚠️ 什么叫【嗅到危险】—— 必须是这些，不是百分比：")
    w("     🔴 驱动链本身出问题：涨价停了/订单没了/扩产取消/催化被证伪")
    w("     🔴 板块见顶信号：连续≥2天资金大额流出 且 板块同时在跌")
    w("     🔴 个股级利空：立案/问询/大股东开始减持/业绩暴雷")
    w("     🔴 从历史最高回落≥5个百分点（市场用价格说这波结束了）")
    w("     ⚪ 『它涨了50%』——★这不是危险，这是成功★")
    w("")
    w("  ★铁律R（2026-08-17 用户亲自更正，推翻旧版）★")
    w("  ⚠️【+10%不是卖出线，是及格线】")
    w("     用户原话：『谁说10%马上跑？我只是说过，一个股票如果连10%")
    w("       都赚不到就让我跑，那叫炒什么股。但我从没说过10%就要跑。")
    w("       如果你看好一个股票或板块，为什么在10%的时候跑？")
    w("       ★赚钱就尽量多赚★，趋势还在，你跑去买别的")
    w("       还不是买回这个行业吗？』")
    w("  ★卖出的唯一依据是【驱动链见顶/破了】，不是百分比")
    w("  ★这和铁律F是同一件事：买入用产业周期的逻辑，")
    w("     卖出就必须用产业周期的标尺。用百分比卖 = 自相矛盾")
    ...
    def _do():
        ...
        for code6, name, tag, cost, stop, sect, chain, mv in WATCH_STOCKS:
            ...
            if peak >= 20 and drop >= 5:
                ...
                line += "  📈 已过及格线(+10%)，★趋势还在就继续拿，不许因为到10%就减★"
            w(line)
        if not _peak_exists:
        ...
        w("    🔴 已删除『+10%减半』『+20%再减半』—— 这两条是错的")
        w("       用户原话：『谁说10%马上跑？我从没说过10%就要跑。")
        w("         如果你看好这个板块，为什么在10%的时候跑？")
        w("         ★赚钱就尽量多赚★，趋势还在，你跑去买别的")
        w("         还不是买回这个行业？』")
        w("    ✅ 唯一的减仓触发：★从历史最高回落≥5个百分点★")
        w("       它不是『锁利』，是【趋势转折的客观信号】")
        w("    ✅ 第二种：驱动链见顶（三条同时成立才算）")
        w("       ①板块连续≥2天资金净流出且累计>30亿")
        w("       ②同时板块在跌(跌>1%)  ③催化被官方/公司证伪")
        w("    ⚠️ 赚了30%、50%，只要①-B驱动还在、没回撤5点 → ★继续拿★")
        w("    ⚠️ +10%是【及格线】不是【卖出线】：")
        w("       『连10%都赚不到就跑，那叫炒什么股』")
        w("  ★铁律F与S不矛盾：")
...
def scan_reco_track():
    ...
    for c6, it in sorted(d.items(), key=lambda x: x[1].get("date", ""), reverse=True):
        try:
            ...
            ch = it.get("chain", "")
            # ★V27.0：区分【推荐】和【观察】
            # 8/21用户当场抓到：山东黄金/紫金/天孚我从没说过"买"，
            #   只是列进候选池观察，跟踪表却算成AI战绩 → 虚报。
            _kind = it.get("kind", "推荐")
            _km = {"推荐": "🎯推荐", "观察": "👁️观察", "否决": "🚫否决"}.get(_kind, _kind)
    ...
    w("")
    w("  ⚠️ ★只有【🎯推荐】才算AI战绩★")
    w("     【👁️观察】= 只放进候选池，从没说过买 → 不计入胜率")
    w("     【🚫否决】= AI主动拦下的 → 涨了算我错，跌了算我对")
    w("     8/21用户当场抓到：我把三只『观察』记成了『推荐』，虚报战绩。")
...
def main():
    ...
    try:
        ...
        w("=" * 60)
        w("  \u26052026-08-21\uff1aAI\u63a8\u8350 1\u80dc7\u8d1f\uff0c\u6839\u56e0\u662f\u7528\u5df2\u5224\u6b7b\u7684\u7b5b\u9009\u5668\u9009\u80a1\u3002")
        w("  \u2605\u4ece\u4eca\u5929\u8d77\uff0c\u53ea\u6709\u4e0b\u9762\u8fd9\u4e9b\u6a21\u5757\u7684\u8f93\u51fa\u53ef\u4ee5\u62ff\u6765\u63a8\u8350\uff1a")
        w("")
        _any = False
        for _k, _v in RULE_WINRATE_TAG.items():
            if _v and _v[0] is not None and _v[1]:
                w("  \U0001f7e2 %s\uff08\u5386\u53f2%.1f%%\uff09" % (_k, _v[0]))
                _any = True
        if not _any:
            w("  \U0001f534 \u4eca\u5929\u6ca1\u6709\u4efb\u4f55\u6a21\u5757\u7684\u80dc\u7387>45%%")
            w("     \u2192 \u2605\u4e0d\u8bb8\u63a8\u8350\u4efb\u4f55\u6807\u7684\u2605\uff08\u94c1\u5f8bD\uff09")
        w("")
        w("  \U0001f534 \u5df2\u505c\u7528\uff08\u4ec5\u4f5c\u8bb0\u5f55\uff0c\u4e0d\u8bb8\u63a8\u8350\uff09\uff1a")
        for _k, _v in RULE_WINRATE_TAG.items():
