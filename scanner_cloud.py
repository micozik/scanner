...
def scan_skeleton_top():
    ...
    w("")
    w("=" * 60)
    w("🎯🎯🎯【用户触发词 · 固定输出模板 · V17.0】🎯🎯🎯")
    w("=" * 60)
    w("  ★★用户2026-08-14定死的工作方式★★")
    w("  用户说：『抓盘中，美股，龙虎榜，干活！』")
    w("  = AI必须从头到尾读完三份报告，按下面【十一节】顺序输出，一节不许少。")
    w("")
    w("  ⚠️为什么要定死顺序：AI三天里漏了六次，每次都是用户发现的。")
    w("    根因不是记性，是【AI在回答问题，不是在完成报告】——")
    w("    问持仓就找持仓，不问新闻就不看新闻。6800行报告只读需要的那几行。")
    w("  ★把『检查有没有漏』从用户脑子里，搬到纸面上。")
    w("")
    w("  ┌──── 固定输出十一节（缺一节 = 失职，用户可当场追责）────┐")
    w("  │ 1️⃣ 今天的情况   —— 一句话总述：今天市场发生了什么")
    w("  │ 2️⃣ 盘中情况     —— 报告时间/数据新鲜度/涨跌家数/活跃度")
    w("  │ 3️⃣ 大盘         —— 风险分/上涨占比/创业板科创50/次日判定")
    w("  │ 4️⃣ 板块         —— ★排名跳升榜 + 资金流向 + 涨幅连涨天数★")
    w("  │                   三样都要，跳升是领先指标排第一")
    w("  │ 5️⃣ 个股         —— 持仓逐只(价格/盈亏/板块/触发线) + 候选池")
    w("  │ 6️⃣ 方向         —— 今天最强的是什么？我在里面吗？不在为什么？")
    w("  │                   ★推演引擎的✅验证信号 + 全板块交叉前5")
    w("  │ 7️⃣ 新闻         —— ★★八类全写，一类不许省★★")
    w("  │                   ①名人 ②国内政策 ③海外政策 ④科技产业")
    w("  │                   ⑤大宗地缘 ⑥资金事件 ⑦消费养殖 ⑧政策专项")
    w("  │                   ★★★⑤-B 美股隔夜必写：费半/个股/聪明钱/宏观")
    w("  │                      + ★三方对照★ 美股涨的板块A股今天跟了没")
    w("  │                   ⚠️用户原话：『新闻很重要，提供方向的』")
    w("  │                     『外国名人、科技公司的新闻你全部忽略了』")
    w("  │ 8️⃣ 资金         —— 板块资金前10/流出前5 + 龙虎榜机构在买什么")
    w("  │                   + 埋伏池(跌着被买) + 主力净流入")
    w("  │ 9️⃣ 推荐         —— 给具体标的：代码/名字/现价")
    w("  │                   ★没有就明说『今天没有』，不许硬凑（铁律D）")
    w("  │ 🔟 操作         —— 买/卖/持有，具体到股数和价格")
    w("  │ 1️⃣1️⃣ 理由      —— 每个操作的①-B驱动/位置/仓位/止损/逻辑破")
    w("  └────────────────────────────────────────┘")
    w("")
    w("  ⚠️ 第7️⃣节新闻是最常漏的，它是①-B的唯一信息来源。")
    w("     不读新闻 → 答不出『这只票靠什么赚钱』→ 所有推荐都是瞎猜。")
    w("  ⚠️ 第9️⃣节答不出就写『答不出』，写空比编一个更有价值。")
    w("=" * 60)
    w("")
    w("🔒🔒🔒【读报告定律 · V9.3 · 不许跳步】🔒🔒🔒")
    w("=" * 60)
