...
RECOMMENDATIONS = [
    # 进行中
    ("2026-07-27", "159796", "电池ETF汇", 0.840, "B", "至9/1消费税(33天)",
     "①消费税取消/延期 ②钠电订单证伪 ③碳酸锂重新单边下跌"),
    ("2026-07-27", "301269", "华大九天", 91.999, "B", "8周(国产EDA替代)",
     "EDA国产化政策转向 或 软件板块持续失血"),
    # 已平仓（归档，战绩不许赖）
    # 2026-07-22 半导体设备ETF @1.180 → 清于1.044  −11.5%  A类超期
    # 2026-07-23 电网设备ETF   @0.854 → 清于0.836  −2.1%   A类超期
    # 2026-07-17 深圳华强      @23.087→ 清于21.17  −8.3%   A类超期
    # 2026-07-27 中科曙光      @89.199→ 清于87.05  −2.4%
    # 2026-07-15 招商轮船      @15.215→ 清于15.68  +3.1%  实现+1040 ✅
]
...
DEDUCTION_CHAINS = [
    {
        ...
        "verify": ["液冷订单", "液冷中标", "冷板量产", "CDU", "浸没"],
    },
    {
        "name": "AI算力 → 供电",
        "trigger": ["AI芯片", "机架", "数据中心", "算力中心", "超节点", "功率"],
        "layers": ["①单机架功率从10kW→100kW", "②传统UPS不够→HVDC/800VDC",
                   "③固态变压器/BBU备电/母线槽", "④SiC功率器件"],
        "stocks": "麦格米特/科华数据/科士达/中恒电气/欧陆通/新雷能",
        "verify": ["800VDC", "HVDC", "固态变压器", "BBU", "电源中标"],
    },
    {
        "name": "先进封装 → 玻璃基板",
        "trigger": ["先进封装", "CoWoS", "2.5D", "3D堆叠", "载板", "基板",
                    "玻璃基", "TGV", "封装产能"],
        "layers": ["①摩尔定律见顶→算力靠堆封装", "②有机基板承载不了大尺寸",
                   "③玻璃基板成下一代载板", "④TGV激光钻孔/电镀/高纯石英玻璃"],
        "stocks": "凯盛科技/沃格光电/德龙激光/帝尔激光/长电科技/通富微电",
        "verify": ["玻璃基板", "TGV", "中试线", "样品", "量产", "投资"],
    },
    {
        "name": "存储涨价 → 传导链",
        "trigger": ["存储", "DRAM", "NAND", "HBM", "内存", "涨价", "缺货",
                    "美光", "海力士", "三星", "长鑫"],
        "layers": ["①AI重塑存储周期→供不应求", "②原厂涨价→模组厂涨价",
                   "③终端涨价(手机/PC)", "④设备/材料需求→扩产"],
        "stocks": "兆易创新/江波龙/德明利/佰维存储/香农芯创/深科技",
        "verify": ["涨价", "提价", "长约", "扩产", "满产", "订单排至"],
    },
    {
        "name": "核电核准 → 设备链",
        "trigger": ["核电", "核准", "华龙一号", "核电机组", "并网", "核岛"],
        "layers": ["①机组核准→3-5年建设期", "②核岛设备招标(蒸发器/压力容器)",
                   "③核级泵阀/管道/锆材", "④后续燃料+运维"],
        "stocks": "中国核电/东方电气/上海电气/江苏神通/纽威股份/应流股份",
        "verify": ["核电中标", "核岛", "核级", "锆", "蒸汽发生器"],
    },
    {
        "name": "特高压 → 设备链",
        "trigger": ["特高压", "电网投资", "输配电", "十五五电网", "柔性直流"],
        "layers": ["①十五五规模翻倍→投资前置", "②换流阀/变压器/GIS招标",
                   "③电缆/绝缘子/组合电器", "④后续配网+储能配套"],
        "stocks": "许继电气/平高电气/国电南瑞/思源电气/特变电工/中国西电",
        "verify": ["特高压中标", "换流阀", "GIS", "电网招标", "开工"],
    },
    {
        "name": "锂电消费税 → 钠电替代",
        "trigger": ["锂电", "消费税", "碳酸锂", "钠电", "钠离子", "储能"],
        "layers": ["①9/1锂电征4%消费税，钠电免税", "②成本差拉大→钠电替代加速",
                   "③钠电正极(层状氧化物)/硬碳负极", "④集流体铝箔替代铜箔"],
        "stocks": "容百科技/振华新材/元力股份/鼎胜新材/华阳股份/传艺科技",
        "verify": ["钠电订单", "钠电投产", "钠电中标", "量产", "签单"],
    },
    {
        "name": "机器人量产 → 零部件",
        "trigger": ["人形机器人", "具身智能", "Optimus", "宇树", "量产"],
        "layers": ["①量产爬坡→零部件放量", "②谐波/行星滚柱丝杠/无框电机",
                   "③灵巧手(微型丝杠/触觉传感)", "④减速器材料+精密加工"],
        "stocks": "绿的谐波/三花智控/鸣志电器/兆威机电/双环传动/贝斯特",
        "verify": ["机器人订单", "定点", "量产", "送样", "产能"],
    },
    {
        "name": "MLCC涨价 → 被动元件",
        "trigger": ["MLCC", "被动元件", "电容", "国巨", "村田", "三星电机"],
        "layers": ["①AI服务器高容MLCC紧缺", "②原厂涨价→渠道跟涨",
                   "③国产替代加速", "④上游陶瓷粉/镍粉"],
        "stocks": "风华高科/三环集团/宏达电子/火炬电子/torch",
        "verify": ["MLCC涨价", "满产", "产能利用率", "订单"],
    },
    {
        "name": "光模块 → 上游光芯片",
        "trigger": ["光模块", "CPO", "800G", "1.6T", "硅光", "光引擎"],
        "layers": ["①AI数据中心互联需求", "②800G/1.6T光模块放量",
                   "③上游光芯片(EML/DFB)+硅光", "④光引擎良率/先进封装"],
        "stocks": "源杰科技/仕佳光子/长光华芯/天孚通信/太辰光",
        "verify": ["光芯片", "EML", "产能", "良率", "扩产", "订单"],
    },
]
...
def scan_deduction(uniq_news, heat_top=None):
    ...
    for ch in DEDUCTION_CHAINS:
        ...
        for tm, t in uniq_news:
            if t[:26] in seen:
                continue
            if any(k in t for k in ch["trigger"]):
                seen.add(t[:26])
                trig.append((tm, t))
            if any(k in t for k in ch["verify"]):
                seen.add(t[:26])
                ver.append((tm, t))
...
def main():
    ...
    w("=" * 60)
    w(f"A股作战扫描器V3.6 | {bj.strftime('%Y-%m-%d %H:%M')} {weekday} | {mode}")
    w("=" * 60)
