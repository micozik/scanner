...
WATCH_STOCKS = [
    ("159611", "电力ETF广", "持仓", 1.080, 0.980, "电力", "电力/核电链", 1.24),
    ("159796", "电池ETF汇", "持仓", 0.840, 0.790, "电池", "锂电/钠电链", 4.66),
    ("000938", "紫光股份", "持仓", 34.681, 29.48, "计算机设备", "AI算力链", 2.98),
    ("301269", "华大九天", "持仓", 91.999, 84.00, "软件开发", "AI算力链", 0.87),
    ("002714", "牧原股份", "持仓", 39.613, 36.50, "养殖业", "农业(独立)", 3.11),
    ("000066", "中国长城", "重点观察", 0, 0, "计算机设备", "AI算力链", 0),
    ("300308", "中际旭创", "观察·机构抄底", 0, 0, "通信设备", "AI算力链", 0),
    ("300502", "新易盛", "观察·机构抄底", 0, 0, "通信设备", "AI算力链", 0),
]
TOTAL_ASSET = 17.5   # 总资产（万元），买卖后AI更新此数
...
RECOMMENDATIONS = [
    ...
    ("2026-07-31", "000938", "紫光股份", 34.681, "B", "12周(算力资本开支)",
     "①北美四大云厂capex指引下调 ②发改委算力网4万亿落空 ③新华三服务器/交换机订单下修"),
    ...
    # 已平仓归档
    # 07-22 半导体设备ETF @1.180→1.044 −11.5% ❌ A类归错(实为B类产业周期)
    # 07-23 电网设备ETF   @0.854→0.836 −2.1%  ❌
    # 07-17 深圳华强      @23.087→21.17 −8.3% ❌
    # 07-27 中科曙光      @89.199→87.05 −2.4% ❌
    # 07-15 招商轮船      @15.215→15.68 +3.1% ✅ 实现+1040
]
...
def main():
    ...
    w("=" * 60)
    w(f"A股作战扫描器V3.8 | {bj.strftime('%Y-%m-%d %H:%M')} {weekday} | {mode}")
    w("=" * 60)
