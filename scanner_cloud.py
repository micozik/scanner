...
def scan_decision_card():
    ...
    w("")
    w("  ④ 位置：60日涨跌 / 均线？")
    w("     跌>12%=低位✅ | 已涨高=追高⚠️ | 跌破MA5=弱")
    w("     → ______________________")
    w("")
    w("  ⑤ 游资：埋伏型 还是 追高型？")
    w("     买『当天在跌』的=埋伏✅高优先 | 买『当天涨停』的=追高⚠️次日易崩")
    w("     → ______________________")
    w("")
    w("  ⑥ 宏观驱动链有没有冲突？")
    w("     油涨→加息预期→杀黄金/杀成长；油跌→利好成长")
    w("     电力/钠电/政策票=独立，不看油脸色")
    w("     → ______________________")
    w("")
    w("  ⑦ 与今天其他建议冲突吗？（多方向不能互相打架）")
    w("     → ______________________")
    w("")
    w("  ─────────────────────────────────────")
    w("  ⚠️ 七项填不满 = 不许出买卖建议")
    w("  ⚠️ 铁律A：有日期的未来催化 > 过去的资金流（因果关系）")
    w("  ⚠️ 铁律B：游资在跌的板块砸钱=埋伏=明天机会")
    w("  ⚠️ 铁律C：先board再stock，板块逆风一票否决")
    w("  ⚠️ 铁律D：宁可报空，不硬凑标的")
    w("  ⚠️ 铁律E：踏空也是亏（看上了没买），方向确认就给进攻方案")
    w("=" * 60)
