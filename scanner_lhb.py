    ...
    try:
        ...
        try:
            ...
            for _r in list(out) + [(x[0], x[2], x[3], x[1], x[4]) for x in _blocked]:
                ...
                # ★★V3.7：记录板块名，供【次日板块是否联动】分组统计★★
                # 用户2026-08-14：『机构潜伏很重要，只要是大资金买入而且诡异的
                #   都有猫腻，配合板块联动，很有可能赚钱』
                # ★现在的成绩单只测了"机构买没买"，没测"板块联不联动"，
                #   而用户说的是【两个条件同时满足】。混在一起算等于测错了东西。
                # 8/13实例：3只埋伏里，麦迪科技(医疗服务→次日医药跳383位)、
                #   杰理科技(半导体→次日+1.24%)板块都启动了；
                #   天娱数科(文化传媒→次日-2.08%资金-26.44亿)没启动。
                _bd = ""
                try:
                    _bd = str(_r[5]) if len(_r) > 5 else ""
                except Exception:
                    _bd = ""
                _arch.append({
                    "board": _bd,
...
def backtest_institution():
    ...
    w("")
    w("")
    w("  ── ★★组合分组（用户命题：机构埋伏 × 金额 × 方向）★★──")
    for _k in ("★买跌+大额(≥1亿)", "★买跌+小额(<1亿)", "★机构+游资同向买跌"):
        _h, _n, _sm = buckets.get(_k, [0, 0, 0.0])
        if _n:
            w(f"    {_k:20s}：{_h:>3}/{_n:<3} = {_h/_n*100:5.1f}%  平均{_sm/_n:+6.2f}%")
        else:
            w(f"    {_k:20s}：样本0，继续积累")
    w("")
    w("  ★用户2026-08-14命题：『大资金买入+诡异+板块联动 = 有猫腻』")
    w("    ⚠️【板块联动】需跨文件比对次日板块涨跌，V3.8接入。")
    w("      当前只能测【机构+游资同向】这个近似条件。")
    w("    ★8/13实例：3只埋伏中，麦迪科技(医疗服务→次日医药跳383位)、")
    w("      杰理科技(半导体→次日+1.24%)板块都启动；")
    w("      天娱数科(文化传媒→次日-2.08%资金-26.44亿)没启动。")
    w("      → 若按【埋伏+板块启动】筛，3只里有2只符合，")
    w("        而现在的回测把3只混在一起算 —— 这可能是在测错东西。")
    w("")
    w("  ── 结论怎么读 ──")
